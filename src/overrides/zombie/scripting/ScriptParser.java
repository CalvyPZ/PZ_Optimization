...
public final class ScriptParser {
   ...
   static {
      pzopt.Overrides.onClassLoaded("zombie.scripting.ScriptParser");
   }
   ...
   public static String stripComments(String totalFile) {
      if (pzopt.Config.SCRIPT_PARSER_FAST && pzopt.Overrides.enabled()) {
         String fast = pzopt.ScriptText.stripComments(totalFile); // pzopt: linear pass; null = unbalanced, use the stock pass
         if (fast != null) {
            return fast;
         }
      }
   ...
   public static ArrayList<String> parseTokens(String totalFile) {
      if (pzopt.Config.SCRIPT_PARSER_FAST && pzopt.Overrides.enabled()) {
         return pzopt.ScriptText.parseTokens(totalFile); // pzopt: index instead of re-substringing the remainder per block
      }
