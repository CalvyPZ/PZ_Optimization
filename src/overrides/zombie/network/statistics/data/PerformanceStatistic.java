...
public class PerformanceStatistic extends Statistic implements IStatistic {
   // pzopt: marker so the game log shows the loose class was loaded, not the jar's copy (quiet: may load before the logger)
   static {
      pzopt.Overrides.onClassLoadedQuiet("zombie.network.statistics.data.PerformanceStatistic");
   }
   ...
   public <PO extends IPooledObject> void onPooledObjectAlloc(Pool<PO> sender, PO newInstance, boolean isReused, PoolStacks threadLocalPoolStacks) {
      PoolCounter counter = this.getPoolCounter(sender, newInstance);
      if (pzopt.PoolStats.ENABLED && pzopt.PoolStats.alloc(counter, threadLocalPoolStacks, isReused)) {
         return; // pzopt: poolStatsBatched, a frame worker's count is published at the end of its batch
      }

      counter.onPooledObjectAlloc(threadLocalPoolStacks, isReused);
   }

   public <PO extends IPooledObject> void onPooledObjectReleased(Pool<PO> sender, PO releasedInstance, PoolStacks threadLocalPoolStacks) {
      PoolCounter counter = this.getPoolCounter(sender, releasedInstance);
      if (pzopt.PoolStats.ENABLED && pzopt.PoolStats.release(counter, threadLocalPoolStacks)) {
         return; // pzopt: poolStatsBatched
      }

      counter.onPooledObjectReleased(threadLocalPoolStacks);
   }
