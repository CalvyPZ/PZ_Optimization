...
public abstract class State implements IAnimEventListener, IAnimEventWrappedBroadcaster, IStateFlagsSource {
   // pzopt: marker so the game log shows the loose class was loaded, not the jar's copy. Quiet: the AI states
   // initialise very early, before the logger the loud marker would re-enter.
   static {
      pzopt.Overrides.onClassLoadedQuiet("zombie.ai.State");
   }
   ...
   private <T> void genericsHelper(IsoGameCharacter character, State.Param<T> param, Object value) {
      character.set(param, (T)value); // pzopt: cast Vineflower dropped (the erased call in the jar)
   }
   ...
   public static class Param<T> {
      ...
      public T get(IsoGameCharacter owner, Supplier<T> defaultSupplier) {
         // pzopt: stateParamMemo. Stock's computeIfAbsent allocates a capturing lambda on every read (the supplier is
         // a parameter), and every AI state reads several of its params per character per frame, so in a horde this
         // was a steady stream of short-lived objects on the game thread on top of two map probes. A get, and a put
         // only when the default was actually produced, is what computeIfAbsent does here: it stores nothing when the
         // mapping function returns null, and it treats a mapping to null as absent.
         if (pzopt.Config.STATE_PARAM_MEMO && pzopt.Overrides.enabled()) {
            Map<State.Param<?>, Object> params = owner.getStateMachineParams(this.fromClazz);
         ...
         return (T)owner.getStateMachineParams(this.fromClazz).computeIfAbsent(this, var1x -> defaultSupplier.get()); // pzopt: cast Vineflower dropped
      }
