...
import zombie.tileDepth.TileGeometryFile.Geometry;
import zombie.tileDepth.TileGeometryFile.Plane;
import zombie.tileDepth.TileGeometryFile.Polygon;
...
public final class TileDepthTexture {
   ...
   void load(float[] pixels, ByteBuffer bb, int stride, int left, int top) {
      ...
      if (pzopt.Config.DEPTH_MAP_FAST && pzopt.Overrides.enabled()) { // pzopt: depthMapFast, a bulk read per row, same values
         this.empty = pzopt.PngFilters.depthTile(pixels, bb, stride, left, top, this.width, this.height);
      } else
      for (int y = 0; y < this.height; y++) {
   ...
   private static void initClampedPixels() {
      if (!clampedPixelsInit) {
         ...
         for (int y = 0; y < 64; y++) {
            for (int x = 0; x < 128; x++) {
               float pixel = TileGeometryUtils.getNormalizedDepthOnPlaneAt(x + 0.5F, y + 0.5F, GridPlane.XZ, planePoint);
               if (pixel >= 0.0F) {
                  pixel += 0.011764706F;
               }
   ...
   void recalculateDepth() {
      ArrayList<Geometry> geometries = TileGeometryManager.getInstance().getGeometry("game", this.getTileset().getName(), this.getColumn(), this.getRow());
      if (geometries != null && !geometries.isEmpty()) {
         ...
         for (int i = 0; i < geometries.size(); i++) {
            ...
            if (geometry.isPolygon()) {
               ((Polygon)geometry).rasterize((tileX, tileY) -> {
   ...
   Geometry getOrCreateFloorPolygon() {
      if (floorPolygon == null) {
         floorPolygon = new Polygon();
         floorPolygon.plane = Plane.XZ;
   ...
   void recalculateShadowDepth() {
      ...
      if (geometries != null && !geometries.isEmpty()) {
         ...
         Geometry floorPolygon = this.getOrCreateFloorPolygon();
