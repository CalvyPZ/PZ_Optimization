...
public final class TileDepthTextures {
   ...
   // pzopt: concurrent, because the load tasks put into it from several threads while the game thread reads it
   private final java.util.concurrent.ConcurrentHashMap<String, TilesetDepthTexture> tilesets = new java.util.concurrent.ConcurrentHashMap<>();
   private final HashSet<String> nullTilesets = new HashSet<>();
   // pzopt: tileset names a load task has claimed, so two tasks never decode the same file
   private final java.util.Set<String> claimedTilesets = java.util.concurrent.ConcurrentHashMap.newKeySet();

   static {
      pzopt.Overrides.onClassLoaded("zombie.tileDepth.TileDepthTextures");
   }
   ...
   static final class LoadTask extends FileTask {
      ...
      public Object call() throws Exception {
         ...
         if (pzopt.Config.PARALLEL_DEPTH_MAPS && pzopt.Overrides.enabled()) {
            // pzopt: stock serialises all 218 tileset decodes (PNG + per-tile GPU upload queueing) under one lock;
            // each tileset is independent, so decode concurrently and only guard against a double load by name
            if (this.textures.tilesets.get(tilesetName) == null && this.textures.claimedTilesets.add(tilesetName)) {
