...
public class ParticleRectangle {
   static {
      pzopt.Overrides.onClassLoaded("zombie.iso.weather.fx.ParticleRectangle");
   }
   ...
   public void render() {
      ...
      if (pzopt.RainTiles.enabled() && !IsoWeatherFX.debugBounds) { // pzopt: template once, one origin per cell
         WeatherParticleDrawer drawer = IsoWeatherFX.instance.getDrawer(this.id);
         pzopt.RainTiles.Tile tile = drawer.pzoptBeginTile();
         ...
         for (int cy = -1; cy < cellsH; cy++) {
            for (int cx = -1; cx < cellsW; cx++) {
               tile.addOrigin((float)(baseX + cx * this.width), (float)(baseY + cy * this.height));
            }
         }
