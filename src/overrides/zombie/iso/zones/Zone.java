...
public class Zone {
   ...
   public void setHaveConstruction(boolean have) {
      ...
      if (GameClient.client) {
         ...
         PacketType.ConstructedZone.doPacket(bb);
         bb.putInt(this.x);
         bb.putInt(this.y);
         bb.putInt(this.z);
   ...
   public Location pickRandomLocation(Location location) {
   ...
   public IsoGridSquare getRandomSquareInZone() {
      Location location = this.pickRandomLocation((Location)IsoMetaGrid.TL_Location.get());
   ...
   public void sendToServer() {
      if (GameClient.client) {
         INetworkPacket.send(PacketType.RegisterZone, new Object[]{this});
      }
   }
      ...
      if (this.polylineOutlinePoints == null) {
         if (this.isPolyline()) {
            if (this.polylineWidth > 0) {
               ...
               clipperBuffer.flip();
               clipperOffset.addPath(this.points.size() / 2, clipperBuffer, JoinType.Miter.ordinal(), EndType.Butt.ordinal());
   ...
   boolean lineSegmentIntersects(float sx, float sy, float ex, float ey) {
      ...
      if (pzopt.Config.ZONE_EDGE_PREFILTER && pzopt.Overrides.enabled()) { // pzopt: zoneEdgePrefilter, far edges skipped, same answers
         if (pzopt.ZoneGeom.polygonEdgesHit(this.points, sx, sy, ex, ey, dirX, dirY, lineSegmentLength)) {
            return true;
         }
      } else
      for (int j = 0; j < this.points.size(); j += 2) {
   ...
   boolean polylineOutlineSegmentIntersects(float sx, float sy, float ex, float ey) {
      ...
      if (pzopt.Config.ZONE_EDGE_PREFILTER && pzopt.Overrides.enabled()) { // pzopt: zoneEdgePrefilter
         if (pzopt.ZoneGeom.outlineEdgesHit(points, sx, sy, ex, ey, dirX, dirY, lineSegmentLength)) {
            return true;
         }
      } else
