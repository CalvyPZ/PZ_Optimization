...
local SECTIONS = {
    ...
    },
    {
        ...
        entries = {
            ...
            { key = "animBonesThreads", label = "Worker threads for that batch",
              choices = { "2", "4", "8", "15" }, tip = "Threads of the zombie bone-math batch (the game thread joins in); more than cores - 1 is clamped." },
            { key = "vehicleCull", label = "Skip far vehicles in the zombie line-of-sight test",
              tip = "A zombie checking whether a car blocks its view of you tests only the cars whose bounding circle reaches the line; stock ran the exact box test on every loaded car (6 % of the game thread downtown)." },
        },
    },
    {
...
local KEY_CLIP = {
    ...
    lightingStrongDelta = "torch", lightingStrongBudget = "horde", lightingFlush = "torch", lightingBudget = "torch",
    lightSwitchCheckFrames = "horde", soundZoneCache = "horde", animBonesParallel = "horde", animBonesThreads = "horde", vehicleCull = "horde", gridStackInterval = "horde",
