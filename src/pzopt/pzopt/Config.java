 ...
 *   chunkGridWidth int       the player's chunk grid (loaded / simulated / rendered chunks around the player) is this many
 *                            chunks wide, 8 tiles each, instead of the stock size from the screen resolution (19 at 1080p
 *                            and above, 13 at 720p); odd, clamped to 5..15. Smaller = less world simulated, lit and baked
 *                            around the player (CPU-bound setups, heavy mod lists) but the world ends nearer the screen
 *                            edge at wide zoom. 0 = stock (default 0; IsoChunkMap.CalcChunkWidth)
...
public final class Config {
   ...
   public static final int CHUNK_GRID_WIDTH = Math.max(0, integer("chunkGridWidth", 0)); // chunks per side of the player's chunk grid; 0 = the stock screen-size value (IsoChunkMap.CalcChunkWidth)
   ...
   public static final String JIT_MODE = string("jitMode", "auto"); // pzopt.JitGovernor: tiered (stock) | c1play (C2 excluded from world start on) | auto (default: c1play on jitC1Cores cores or fewer)
