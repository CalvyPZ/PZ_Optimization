#!/bin/bash
# Side-by-side of the two heavy-fog thunderstorm drive recordings (preset storm-fog: STAGE_STORM + fog 1.0), aligned at the car's motion onset, with the
...
set -e
cd "$(dirname "$0")/.."
S=harness/runs/sbs-stormfog-stock-20260921-125922/recording.mp4
O=harness/runs/sbs-stormfog-opt-20260921-130035/recording.mp4
S0=15.14   # 2 s before the stock motion onset (17.14, showcase-times.py)
O0=14.95   # 2 s before the optimized motion onset (16.95)
DUR=42
FONT=/usr/share/fonts/TTF/DejaVuSans-Bold.ttf
[ -f "$FONT" ] || FONT=$(fc-match -f '%{file}' 'DejaVu Sans:bold')
OUT=${1:-docs/media/drive-120kmh-storm-fog-stock-vs-optimized.mp4}
mkdir -p "$(dirname "$OUT")"
...
drawtext=fontfile=$FONT:fontsize=34:fontcolor=0xb4b4b8:x=20:y=18:text='STOCK  (every optimization off, in-game cap 300)   60 fps mean, p99 72 ms, 49 frames over 33 ms'[l];\
...
drawtext=fontfile=$FONT:fontsize=34:fontcolor=0xb4b4b8:x=20:y=18:text='OPTIMIZED + fog pass  (uncapped)   191 fps mean, p99 16.5 ms, 0 frames over 33 ms'[r];\
[l][r]hstack=inputs=2,drawbox=y=780:h=30:w=3840:color=black@0.75:t=fill,\
drawtext=fontfile=$FONT:fontsize=22:fontcolor=0x8a8a90:x=(w-text_w)/2:y=784:text='Project Zomboid B42.20  thunderstorm in heavy fog, 120 km/h, 5120x2160 max zoom, RTX 4090 / 9800X3D  -  PZ_Optimization 2026-09-21',setparams=color_primaries=bt2020:color_trc=smpte2084:colorspace=bt2020nc:range=tv" \
